...
void VL53L0X::setWire(SoftwareWire* _wireObj)
{
  wireObj = _wireObj;
}
...
// mode.
bool VL53L0X::init(SoftwareWire* _wireObj, bool io_2v8)
{
...
{
  wireObj->beginTransmission(address);
  wireObj->write(reg);
  wireObj->write(value);
  last_status = wireObj->endTransmission();
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  wireObj->beginTransmission(address);
  wireObj->write(reg);
  wireObj->write((value >> 8) & 0xFF); // value high byte
  wireObj->write( value       & 0xFF); // value low byte
  last_status = wireObj->endTransmission();
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  wireObj->beginTransmission(address);
  wireObj->write(reg);
  wireObj->write((value >> 24) & 0xFF); // value highest byte
  wireObj->write((value >> 16) & 0xFF);
  wireObj->write((value >>  8) & 0xFF);
  wireObj->write( value        & 0xFF); // value lowest byte
  last_status = wireObj->endTransmission();
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  wireObj->requestFrom(address, (uint8_t)1);
  value = wireObj->read();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  wireObj->requestFrom(address, (uint8_t)2);
  value  = (uint16_t)wireObj->read() << 8; // value high byte
  value |=           wireObj->read();      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  wireObj->requestFrom(address, (uint8_t)4);
  value  = (uint32_t)wireObj->read() << 24; // value highest byte
  value |= (uint32_t)wireObj->read() << 16;
  value |= (uint16_t)wireObj->read() <<  8;
  value |=           wireObj->read();       // value lowest byte
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  wireObj->requestFrom(address, count);
